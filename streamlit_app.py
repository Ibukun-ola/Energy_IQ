...
if predict_btn:
    ...
    st.markdown('<div class="section-label">Energy Balance</div>', unsafe_allow_html=True)

    fig = go.Figure()
    fig.add_trace(go.Bar(
        name='Grid Supply',
        x=['Daily Energy Balance'],
        y=[grid_supply_kwh],
        marker_color='#4cff8f',
        text=f'{grid_supply_kwh:.1f} kWh',
        textposition='inside',
        textfont=dict(color='#0a0f0d', size=13, family='Syne')
    ))
    fig.add_trace(go.Bar(
        name='Diesel Requirement',
        x=['Daily Energy Balance'],
        y=[diesel_demand_kwh],
        marker_color='#ff6b35',
        text=f'{diesel_demand_kwh:.1f} kWh',
        textposition='inside',
    ...
    fig.update_layout(
        ...
        yaxis=dict(gridcolor='#1e2e21', title='kWh'),
        xaxis=dict(gridcolor='#1e2e21')
    )
    ...
    with col_d1:
        fig2 = go.Figure(go.Pie(
            ...
            labels=['Grid (Clean)', 'Diesel (Generator)'],
            hole=0.65,
            marker_colors=['#4cff8f', '#ff6b35'],
            textinfo='percent',
